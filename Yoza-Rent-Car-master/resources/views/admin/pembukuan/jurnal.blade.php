...
<x-page-header :title="__('Jurnal Harian')" :description="__('Catatan double-entry otomatis dari setiap transaksi.')" />

{{-- Filter --}}
<form method="GET"
      class="mb-4 flex flex-wrap gap-2 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
    <x-input name="tanggal_dari" type="date" :label="__('Dari')"
        :value="request('tanggal_dari')" class="flex-1 min-w-[140px]" />
    <x-input name="tanggal_sampai" type="date" :label="__('Sampai')"
        :value="request('tanggal_sampai')" class="flex-1 min-w-[140px]" />
    <x-select name="account_id" :label="__('Akun')"
        :options="$accounts->pluck('nama','id')->toArray()"
        :selected="request('account_id')"
        :placeholder="__('Semua akun')"
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody>
                ...
                <tr>
                    <td colspan="5">
                        <x-empty-state icon="book-open" :title="__('Tidak ada entri jurnal')"
                            :description="__('Jurnal akan dibuat otomatis saat ada transaksi.')" />
                    </td>
                </tr>
                @endforelse
            </tbody>
